...
def plot_cities(cities_tups, lines=False, filename=None):
    ...
    if not lines:
        plt.plot(*zip(*cities_tups), linestyle='none')
    else:
        plt.plot(*zip(*cities_tups), linestyle='-', color='gray', linewidth=1)
    if filename is None:
        plt.show()
    else:
...
def produce_final(file="ulysses16.tsp"):
    ...
    return list(map(lambda s: (float(s[0]), float(s[1])), cities_tups))


if __name__ == '__main__':
    print(list(produce_final('tsp/kroA100.tsp')))
